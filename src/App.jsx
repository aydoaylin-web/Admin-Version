...
/* Gibt es einen Spielstand, bei dem sich ein "Weitermachen?" lohnt?
   Diese Funktion wurde aufgerufen, aber nie geschrieben - daher der
   Fehler "hasSavedProgress is not defined" bei jedem Zurueckwechseln
   zum Tab. Die Bedingung ist dieselbe wie beim ersten Rendern, damit
   beide Stellen nicht wieder auseinanderlaufen koennen. */
function hasSavedProgress(state) {
  const s = state || loadState();
  if (s.introSeen !== true) return false;
  return ((s.completed?.length) || 0) > 0
    || (s.score ?? 0) !== 0
    || ((s.caseResults?.length) || 0) > 0;
}
...
export default function App({ contentOverride = null, previewMode = false, onOpenAdmin = null }) {
  ...
  const [showResume,setShowResume]=useState(()=>hasSavedProgress(saved));
  ...
  const loseScore=Number(contentSettings.loseScore) || -10;
  const defaultTimeLimit=Number(contentSettings.defaultTimeLimit) || 180;
  const maxTips=Number(contentSettings.maxTips) || MAX_TIPS;
  const notificationDelayMin=Number(contentSettings.notificationDelayMin) || 12000;
  const notificationDelayMax=Number(contentSettings.notificationDelayMax) || 25000;
  const notificationHistoryLimit=Number(contentSettings.notificationHistoryLimit) || 20;
  const endWhenAllTasksCompleted=contentSettings.endWhenAllTasksCompleted !== false;
  ...
  useEffect(() => {
    ...
    async function applyContent() {
      ...
      try {
        const pack = contentOverride
          ? {
              posts: contentOverride.posts || [],
              tasks: contentOverride.tasks || [],
              profiles: contentOverride.profiles || [],
              stories: contentOverride.stories || [],
              guides: contentOverride.guides || [],
              settings: contentOverride.settings || {},
              manifest: contentOverride.manifest || null,
            }
        ...
        const followed = new Set(sourceTasks.map((task) => task.followUpTaskId).filter(Boolean));
        const pushTaskIds = new Set(sourcePosts.map((post) => post.taskId).filter(Boolean));
        const primary = sourceTasks
          .filter((task) => pushTaskIds.has(task.id) && !followed.has(task.id))
          .map((task) => task.id);

        const postsWithSavedComments = sourcePosts.map((post) => ({
          ...post,
          comments: [
            ...(post.comments || []),
            ...(previewMode ? [] : (saved.customComments?.[post.id] || [])),
          ],
        }));
        ...
        setRunOrder((current) =>
          current.length
            ? current
            : settings.randomizePrimaryMissions === false
              ? primary
              : shuffle(primary),
        );
        setLoading(false);
  ...
  useEffect(()=>{ if(!activeTask||feedback)return; setSeconds(Number(activeTask.timeLimit)||defaultTimeLimit); const id=setInterval(()=>setSeconds(v=>Math.max(0,v-1)),1000); return()=>clearInterval(id); },[activeTask,feedback,defaultTimeLimit]);
  ...
  useEffect(() => {
    ...
    let timeoutId;
    const scheduleNext = () => {
      const delay = notificationDelayMin + Math.random() * Math.max(0, notificationDelayMax - notificationDelayMin);
        ...
        if (!activeNotification && !activeTask) {
          ...
          if (candidateId) {
            ...
            if (task && post) {
              ...
              setNotificationHistory(items => [histItem, ...items.filter(i => i.taskId !== task.id)].slice(0, notificationHistoryLimit));
  ...
  const hintButtonLabel=freeHintRounds
    ? `💡 ${t('hintShowFree')}`
    : tipsRemaining>0
      ? `💡 ${t('hintUse')} (${tipsRemaining}/${maxTips})`
      : t('hintNoneLeft');
  ...
  async function submitTask(expired=false){if(!activeTask||feedback||evaluating)return;const open=activeTask.answerMode==='openAnalysis';if(!expired&&activeTask.type==='news'&&!verdict){setFeedback({validation:true,text:t('valChooseVerdict')});return;}if(!expired&&!open&&!selectedAnswers.length){setFeedback({validation:true,text:t('valSelectAnswer')});return;}if(!expired&&(open||activeTask.answerMode==='singleChoiceWithReason')&&reason.trim().length<(activeTask.minimumReasonLength||0)){setFeedback({validation:true,text:t('valTypeClue')});return;}const isNews=activeTask.type==='news'&&!!activeTask.correctVerdict;const reasonEval=(isNews&&reason.trim())?matchReason(activePost?.id||activeTask.postId,reason):null;const verdictCorrect=!isNews||verdict===activeTask.correctVerdict;let correct,delta,point1=0,point2=0;if(isNews){point1=(!expired&&verdictCorrect)?1:0;point2=(!expired&&reasonEval&&reasonEval.matched)?1:0;delta=point1+point2;correct=!expired&&verdictCorrect;}else{const reasoningCorrect=isCorrectAnswer(activeTask,selectedAnswers);correct=!expired&&verdictCorrect&&reasoningCorrect;delta=correct?activeTask.pointsCorrect:activeTask.pointsWrong;}setScore(v=>v+delta);setCompleted(items=>[...new Set([...items,activeTask.id])]);const responseTimeMs=taskStartedAt.current?Date.now()-taskStartedAt.current:null;setCaseResults(items=>[...items.filter(i=>i.taskId!==activeTask.id),{taskId:activeTask.id,postId:activeTask.postId,title:activeTask.title,type:activeTask.type,correct,delta,answers:selectedAnswers,verdict,analysisTools:usedTools,reason,confidence,responseTimeMs,verdictCorrect,reasonMatched:point2===1,matchedConceptId:reasonEval?.conceptId||null,point1,point2}]);if(correct&&activeTask.type==='realityDefense'){const rule=activeTask.ruleToSaveFromAnswer?reason.trim():activeTask.ruleToSave;if(rule)setAgencyRules(items=>[...new Set([...items,rule])]);}logResearchEvent('task_submitted',{taskId:activeTask.id,postId:activeTask.postId,origin:taskOrigin,correct,points:delta,answers:selectedAnswers.join('|'),verdict,correctVerdict:activeTask.correctVerdict||'',analysisTools:usedTools.join('|'),reason,confidence,responseTimeMs,timeRemaining:seconds,verdictCorrect,reasonMatched:point2===1,matchedConceptId:reasonEval?.conceptId||'',matchedVia:reasonEval?.via||'',reasonHadSlangOnly:reasonEval?.hadSlangOnly||false,point1,point2});
    ...
    if(projectedScore>=targetScore||projectedScore<=loseScore||(endWhenAllTasksCompleted&&projectedCompleted.length>=tasks.length)){setRunSummary({score:projectedScore,correct:caseResults.filter(item=>item.correct).length+(correct?1:0),total:projectedCompleted.length,targetReached:projectedScore>=targetScore,lost:projectedScore<=loseScore});}
  ...
  function startNewRun(){
    ...
    setScore(0);setCompleted([]);setCaseResults([]);setAgencyRules([]);setTipsRemaining(maxTips);setRevealedHints([]);setUnreadNotificationCount(0);setNotificationHistory([]);setRunSummary(null);setActiveTab('feed');
